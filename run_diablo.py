...
def run(cfg: DictConfig):
    ...
    # from omni.isaac.kit import SimulationApp # don't include anything else from omni before creating simulation_app
    ...
    # SM = ROS2_SimulationManager(cfg, simulation_app) # handles loading of OmniLRS environment
    ...
    i = 0
    SM.timeline.play()
    while SM.simulation_app.is_running():
        SM.world.step(render=True)
        if SM.world.is_playing():
            # OmniLRS stuff
            if cfg["mode"]["name"] == "ROS2":
                if SM.world.current_time_step_index == 0:
                    SM.world.reset()
